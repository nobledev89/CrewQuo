...
export default function TimesheetsPage() {
  ...
  const handleDownloadAllTimesheets = () => {
    const rows: string[][] = [];
    
    // Header row
    rows.push(['Subcontractor', 'Project', 'Status', 'Date', 'Type', 'Description', 'Regular Hours', 'OT Hours', 'Quantity', 'Amount', 'Notes']);
    ...
    filteredTimesheets.forEach(timesheet => {
      ...
      timesheet.timeLogs.forEach(log => {
        ...
        rows.push([
          ...
          timesheet.project?.name || 'Unknown',
    ...
    const fileName = `timesheets_${filter}_${new Date().toISOString().split('T')[0]}.csv`;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
          <div className="flex flex-wrap items-center justify-between gap-4">
            ...
            {userRole === 'ADMIN' && filteredTimesheets.length > 0 && (
              <button
                onClick={handleDownloadAllTimesheets}
                className="flex items-center gap-2 px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition font-medium"
              >
                <Download className="w-4 h-4" />
                Download All ({filteredTimesheets.length})
              </button>
            )}
          </div>
        </div>
              ...
              <div key={timesheet.submission.id} className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                ...
                <div className="bg-gradient-to-r from-blue-50 to-indigo-50 px-6 py-4 border-b border-gray-200">
                  ...
                  <div className="mt-4 flex items-center justify-between">
                    ...
                    {userRole === 'ADMIN' && (
                      <button
                        onClick={() => handleDownloadTimesheet(timesheet)}
                        className="flex items-center gap-1 px-3 py-1.5 text-xs bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition font-medium"
                      >
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="bg-gray-50 border-b border-gray-200">
                        ...
                        <th className="px-4 py-3 text-left text-xs font-semibold text-gray-700 min-w-[200px]">Notes</th>
                      ...
                      {timesheet.timeLogs.map(log => {
                        ...
                        return (
                          <tr key={`log_${log.id}`} className="hover:bg-gray-50">
                            ...
                            <td className="px-4 py-4 text-right text-sm font-semibold text-gray-900">£{log.subCost.toFixed(2)}</td>
                            <td className="px-4 py-4 text-sm text-gray-700">
                              ...
                              {isEditingNote && (
                                <div className="mt-2 space-y-2">
                                  <textarea
                                    value={rejectionNotes[currentNoteKey] || ''}
                                    onChange={(e) => setRejectionNotes({ ...rejectionNotes, [currentNoteKey]: e.target.value })}
                                  ...
                                  <div className="flex gap-1">
                                    <button
                                      onClick={() => handleAddLineNote(timesheet.submission.id, log.id, 'timeLog', rejectionNotes[currentNoteKey] || '')}
                                      disabled={processingId !== null || !rejectionNotes[currentNoteKey]?.trim()}
                      ...
                      {timesheet.expenses.map(exp => {
                        ...
                        return (
                          <tr key={`exp_${exp.id}`} className="hover:bg-gray-50">
                            <td className="px-4 py-4 text-sm text-gray-600">{dateStr}</td>
                            <td className="px-4 py-4 text-sm font-medium text-gray-900">Expense</td>
                            <td className="px-4 py-4 text-sm text-gray-900">{exp.category}</td>
                            <td className="px-4 py-4 text-center text-sm text-gray-900">{(exp.quantity || 1).toFixed(1)}</td>
                            <td className="px-4 py-4 text-right text-sm font-semibold text-gray-900">£{exp.amount.toFixed(2)}</td>
                            <td className="px-4 py-4 text-sm text-gray-700">
                              {notes.length > 0 ? (
                                <div className="space-y-1">
                                  {notes.map((note, idx) => (
                              ...
                              {isEditingNote && (
                                <div className="mt-2 space-y-2">
                                  ...
                                  <div className="flex gap-1">
                                    <button
                                      onClick={() => handleAddLineNote(timesheet.submission.id, exp.id, 'expense', rejectionNotes[currentNoteKey] || '')}
                                      disabled={processingId !== null || !rejectionNotes[currentNoteKey]?.trim()}
                                      className="px-2 py-1 text-xs bg-blue-600 text-white rounded hover:bg-blue-700 disabled:opacity-50"
                                    >
                                      Save
