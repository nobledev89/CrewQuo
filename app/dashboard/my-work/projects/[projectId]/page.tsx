...
export default function ProjectDetailPage() {
  ...
  const expenseOptions =
    payCard?.expenses?.map((e: any) => ({
      ...
      rateType: e.rateType || 'CAPPED', // Default to CAPPED for backward compatibility
      ...
      if (calculationBreakdown.length > 1) {
        ...
        for (const [index, segment] of calculationBreakdown.entries()) {
          ...
          const timeLogData: any = {
            ...
            status,
            startTime: logForm.startTime || null,
    ...
    try {
      ...
      for (const log of outdatedLogs) {
        ...
        if (matchingRates.length === 0) {
          console.warn(`[Recalculate] No matching rates found for role: ${log.roleName}`);
