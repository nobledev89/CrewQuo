...
if (!command || !email) {
  console.log(`
Usage:
  Grant super admin:  npx ts-node scripts/make-super-admin.ts grant <email>
  Revoke super admin: npx ts-node scripts/make-super-admin.ts revoke <email>

Examples:
  npx ts-node scripts/make-super-admin.ts grant admin@example.com
  npx ts-node scripts/make-super-admin.ts revoke admin@example.com
  `);
  process.exit(1);
}
