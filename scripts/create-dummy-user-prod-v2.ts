...
// 1. Service account key file (firebase-service-account.json)
...
if (!admin.apps.length) {
  ...
  if (fs.existsSync(serviceAccountPath)) {
    ...
    console.log('ℹ️  If this fails, download service account key from Firebase Console:');
    console.log('   1. Go to: https://console.firebase.google.com/project/projects-corporatespec/settings/serviceaccounts/adminsdk');
    console.log('   2. Click "Generate new private key"');
    console.log('   3. Save as firebase-service-account.json in project root');
    console.log('');
    
    // Try with just project ID - will use Application Default Credentials or environment
    admin.initializeApp({
      projectId: 'projects-corporatespec',
    });
  }
